...
public class CompanyFacade implements CouponClientFacade  {
		...
		public CouponClientFacade login(String name, String password, UserType userType) throws MyException {
			if(companyDAO.login(name, password)) {
				...
				System.out.println("login comapny successed!");
				return this;
				
			}
			System.out.println("login company failed");

			return null;
		}
		
		/**
		 * This method is used to add to specific company new coupons.
		 * It first creates the coupon and if the creation process was successful
		 * it ties the coupon to the company.
		 * @param coupon Coupon that is going to be added to the specified company.
		 * @param companyId Company to which the coupon is going to be added.
		 * @throws MyException This exception is going to be thrown if there is a problem with either
		 * connection to the server, database errors, coupon already exists or company already owns specified coupon.
		 */
		...
		public ArrayList<Coupon> getAllCoupons() throws MyException {
			return companyDAO.getAllCoupons(this.company.getId());
		}
